...
namespace com.IvanMurzak.Unity.MCP.Editor.Tests
{
    ...
    public static class SystemToolExposureBatchRunner
    {
        ...
        class Callback : ICallbacks
        {
            ...
            {
                Debug.Log($"[SystemToolExposureBatchRunner] RunStarted total={Count(testsToRun)}");
            }

            public void RunFinished(ITestResultAdaptor result)
            {
                Debug.Log($"[SystemToolExposureBatchRunner] RunFinished status={result.TestStatus} total={Count(result.Test)} failCount={result.FailCount}");
            ...
            {
                if (!test.IsSuite)
                    Debug.Log($"[SystemToolExposureBatchRunner] TestStarted {test.FullName}");
            }

            public void TestFinished(ITestResultAdaptor result)
            {
                if (!result.Test.IsSuite)
                    Debug.Log($"[SystemToolExposureBatchRunner] TestFinished {result.Test.FullName} => {result.TestStatus}");
            }
        ...
        {
            Debug.Log("[SystemToolExposureBatchRunner] Starting executeMethod runner");
        ...
        static void WaitForDone()
        {
            ...
            EditorApplication.update -= WaitForDone;
            Debug.Log($"[SystemToolExposureBatchRunner] Exiting with {_callback.ExitCode}");
